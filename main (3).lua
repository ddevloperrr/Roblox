...
local A=u.Object:FindFirstChild"t"
if A then
A:Destroy()
end
...
warn(string.format("[ UX] Failed to load custom asset '%s': %s",J,tostring(M)))
H:Destroy()
...
if not L then
warn("[ UX]  '"..identifyexecutor().."' doesnt support the URL Images. Error: "..M)
...
return[[
{
    ...
    "version": "1.6.54",
    "main": "./dist/main.lua",
    "repository": "https://github.com/Footagesus/UOI",
...
local aj=a.load'k'.Init(nil,ag.UOI.ScreenGui.KeySystem)
...
local h=ag.UOI.Services[g.Type]
if h then
local i={}
...
local ag=a.load'k'.Init(nil,ae.UOI.ScreenGui.Popups)
...
ab=ae
ac.Folder=ab.Folder
ac.Path="Sy/"..tostring(ac.Folder).."/config/"

if not isfolder("Sy/"..ac.Folder)then
makefolder("Sy/"..ac.Folder)
if not isfolder("Sy/"..ac.Folder.."/config/")then
makefolder("Sy/"..ac.Folder.."/config/")
end
end
...
warn"[ UX.ConfigManager ] The config system doesn't work in the studio."
return nil
end
...
aq.Load(al,ap.Content,aq.Elements,ak.Window,ak.UOI,function()
...
warn("[ UOI ] Failed to apply pending config for '"..ar.Flag.."': "..tostring(ay))
end
end)
end
end
...
ao.ElementsModule.Load(ao,ao.UIElements.ContainerFrame,ao.ElementsModule.Elements,Window,UOI,nil,ao.ElementsModule,an)
...
if ar.Folder then
if not isfolder("UOI/"..ar.Folder)then
makefolder("UOI/"..ar.Folder)
end
if not isfolder("UOI/"..ar.Folder.."/assets")then
makefolder("UOI/"..ar.Folder.."/assets")
end
...
ImageTransparency=ar.Transparent and aq.UOI.TransparencyValue or 0
},{
l
})
end
...
ar.Destroyed=true
task.wait(0.4)
aq.UOI.ScreenGui:Destroy()
aq.UOI.NotificationGui:Destroy()
aq.UOI.DropdownGui:Destroy()
...
ar.Transparent=r
aq.UOI.Transparent=r

ar.UIElements.Main.Background.ImageTransparency=r and aq.UOI.TransparencyValue or 0
...
return aq.UOI.UIScale
end

function ar.SetUIScale(p,r)
aq.UOI.UIScale=r
ak(aq.UOI.ScreenGui.UIScale,.2,{Scale=r},Enum.EasingStyle.Quint,Enum.EasingDirection.Out):Play()
return ar
end
...
local p=a.load'S'
local r=a.load'T'
local u=p.Init(ar,aq.UOI,aq.Parent.Parent.ToolTips)
...
return r.New(A,ar.UIElements.SideBar.Frame,ar.Folder,aq.UOI.UIScale,ar)
end
...
local R=J.AbsoluteContentSize.X/aq.UOI.UIScale
local S=L.AbsoluteSize.X/aq.UOI.UIScale
...
local Y=X.AbsoluteSize.X/aq.UOI.UIScale
if Y<V then
V=Y
U=X
end
